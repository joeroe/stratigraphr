...
strg_box_render <- function(strg, n = NULL, max_label_width = 8) {
  ...
  col_pos <- strg_box_compute_col_positions(
    full_layout, labels, max_label_width, n_nodes
  )
  node_to_row <- strg_box_compute_label_rows(
    full_layout, nrow(full_layout)
  )
  ...
  if (nrow(extd_edges) > 0) {
    edge_ops <- strg_box_generate_edge_ops(
      extd_edges, node_to_row, col_pos
    )
    if (nrow(edge_ops) > 0) {
  ...
  label_ops <- strg_box_generate_label_ops(
    labels, node_to_row[1:n_nodes],
    col_pos[1:n_nodes], max_label_width
  )
  ...
  if (nrow(full_layout) > n_nodes) {
    dummy_ops <- strg_box_dummy_ops(
      node_to_row, col_pos, n_nodes
    )
    if (nrow(dummy_ops) > 0) {
...
#' @noRd
strg_box_compute_col_positions <- function(
  layout, labels, max_label_width, n_original
) {
  n_total <- nrow(layout)
...
strg_box_compute_label_rows <- function(layout, n_nodes) {
  ...
  node_to_label_row <- rep(
    label_rows,
    times = purrr::map_int(layer_groups, length)
  )
...
#' @noRd
strg_box_generate_edge_ops <- function(
  edges, node_to_row, col_pos
) {
  ...
  all_ops <- purrr::map2(
    ...
    layers[-1],
    function(parent_layer, child_layer) {
      parent_nodes <- names(node_to_row)[
        node_to_row == parent_layer
      ]
      child_nodes <- names(node_to_row)[
        node_to_row == child_layer
      ]
      edge_row <- parent_layer + 1

      strg_box_layer_edge_ops(
        parent_nodes, child_nodes,
        children, col_pos, edge_row
      )
    }
  )
...
#' @noRd
strg_box_layer_edge_ops <- function(
  parent_nodes, child_nodes, children,
  col_pos, edge_row
) {
  connections <- strg_box_build_connections(
    parent_nodes, child_nodes, children, col_pos
  )
  ...
  purrr::map_dfr(all_cols, function(col) {
    ...
    if (!is.null(char)) {
      data.frame(
        row = edge_row, col = col,
        char = char, stringsAsFactors = FALSE
      )
    } else {
      data.frame(
        row = integer(), col = integer(),
        char = character(), stringsAsFactors = FALSE
      )
    }
  })
}
...
#' @noRd
strg_box_build_connections <- function(
  parent_nodes, child_nodes, children, col_pos
) {
...
strg_box_column_span_info <- function(col, spans) {
  in_span <- any(purrr::map_lgl(
    spans, ~ col >= .x$min_col && col <= .x$max_col
  ))
  at_left_end <- any(purrr::map_lgl(
    spans, ~ col == .x$min_col && col <= .x$max_col
  ))
  at_right_end <- any(purrr::map_lgl(
    spans, ~ col == .x$max_col && col >= .x$min_col
  ))

  list(
    in_span = in_span,
    at_left_end = at_left_end,
    at_right_end = at_right_end
  )
}

# Compute horizontal spans for edge drawing
#' @noRd
strg_box_edge_spans <- function(
  connections, has_parent_above, has_child_below
) {
...
#' @noRd
strg_box_generate_label_ops <- function(
  labels, node_to_label_row, col_pos, max_label_width
) {
  purrr::pmap_dfr(
    list(
      label = labels,
...
#' @noRd
strg_box_dummy_ops <- function(
  node_to_row, col_pos, n_original
) {
...
strg_box_canvas_to_lines <- function(canvas, n, node_levels, n_nodes) {
  ...
  if (is.null(n) || n >= total_levels) {
    ...
    # Each level takes 2 rows (label row + edge row),
    # except last level has no edge row.
