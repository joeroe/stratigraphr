...
data from Shubayqa 1 (Richter et al. 2017). The latter is made available
by the c14 package:
...
shub1_c14 |>
  mutate(r_date = cql_r_date(lab_id, c14_age, c14_error)) |>
  pluck("r_date") |>
  cql()
...
shub1_c14 |>
  filter(!outlier) |>
  group_by(phase) |>
  summarise(cql = cql_phase(phase, cql_r_date(lab_id, c14_age, c14_error))) |>
  arrange(desc(phase)) |>
